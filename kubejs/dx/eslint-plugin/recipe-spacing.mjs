...
export default ESLintUtils.RuleCreator.withoutDocs({
    meta: {
        ...
        messages: {
            'space-before-result':
                "There should not be any space between '(' and the recipe result.",
            'space-before-pattern':
                "There should be exactly 1 space between the recipe result and and '['.",
            'space-before-ingredients':
                "There should be exactly 1 space between ']' and '{'.",
            'space-after-ingredients':
                "There should not be any space between the ingredient list and ')'.",
        },
        schema: [],
    },
    defaultOptions: [],
        ...
        return {
            CallExpression(node) {
                ...
                if (
                    ...
                    ingredients.type !== 'ObjectExpression'
                )
                    return;
                    ...
                    context.report({
                        ...
                        fix: (fixer) => [
                            fixer.replaceTextRange(
                                [result.range[1], pattern.range[0]],
                                ', '
                            ),
                        ],
                    });
                ...
                if (
                    ...
                    context.report({
                        ...
                        fix: (fixer) => [
                            fixer.replaceTextRange(
                                [pattern.range[1], ingredients.range[0]],
                                ', '
                            ),
                        ],
                    });
