...
MIMachineEvents.registerRecipeTypes(event => {
    SUPERCOMPUTER = event.register('supercomputer')
        .withItemInputs()
        .withItemOutputs()
        .withFluidInputs();
});
...
MIMachineEvents.registerMachines(event => {
    ...
    const skySteelBlock = event.memberOfBlock(me('sky_steel_block'));
    const skyStoneStairs = event.memberOfBlock(ae('smooth_sky_stone_stairs'));
    const resonanceGenerator = event.memberOfBlock(ae('crystal_resonance_generator'));
    const computerHatch = event.hatchOf('item_input', 'item_output', 'energy_input', 'fluid_input');

    const superComputerBuilder = event.layeredShape('sky_steel_machine_casing', [
        ['  CCC  ', '       ', '       ', '       ', '       ', '  CCC  '],
