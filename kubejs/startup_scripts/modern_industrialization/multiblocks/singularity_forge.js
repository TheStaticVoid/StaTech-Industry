...
MIMachineEvents.registerRecipeTypes((event) => {
    SINGULARITY_FORGE = event
        .register('singularity_forge')
        .withItemInputs()
        .withItemOutputs()
        .withFluidInputs();
});
...
MIMachineEvents.registerMachines((event) => {
    const singularityForgeHatch = event.hatchOf(
        'item_input',
        'item_output',
        'energy_input',
        'fluid_input'
    );
    const plasmaCasing = event.memberOfBlock(
        mi('plasma_handling_iridium_machine_casing')
    );
    const iridiumPipeCasing = event.memberOfBlock(
        mi('iridium_machine_casing_pipe')
    );
    const glowingCaloritePillar = event.memberOfBlock(
        kj('glowing_calorite_pillar')
    );
    ...
    const singularityForgeShape = event
    ...
    event.simpleElectricCraftingMultiBlock(
        ...
        (itemInputs) => itemInputs.addSlots(25, 35, 2, 2),
        (itemOutputs) => itemOutputs.addSlot(102, 35),
        (fluidInputs) => fluidInputs.addSlot(25, 71),
        (fluidOutputs) => {},
