...
MIMachineEvents.registerRecipeTypes((event) => {
    SPACE_PROBE_LAUNCHER = event
        .register('space_probe_launcher')
        .withItemInputs()
        .withItemOutputs();
});

MIMachineEvents.registerMachines((event) => {
    const stainlessCasing = event.memberOfBlock(
    ...
    const spaceProbeHatch = event.hatchOf(
        'item_input',
        'item_output',
        'energy_input'
    );

    const spaceProbeBuilder = event
