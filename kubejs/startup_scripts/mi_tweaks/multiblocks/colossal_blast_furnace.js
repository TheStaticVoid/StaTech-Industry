...
MIMachineEvents.registerRecipeTypes((event) => {
    COLOSSAL_BLAST_FURNACE = event
        .register('colossal_blast_furnace')
        .withItemInputs()
        .withItemOutputs();
});
...
MITweaksMachineEvents.registerBatchMultiblocks((event) => {
    ...
    // Block Members
    const modernIndustrializationOstrumMachineCasing = event.memberOfBlock(
        'modern_industrialization:ostrum_machine_casing'
    );
    const modernIndustrializationCaloriteMachineCasingPipe =
        event.memberOfBlock(
            'modern_industrialization:calorite_machine_casing_pipe'
        );
    const kubejsGlowingCaloritePillar = event.memberOfBlock(
        'kubejs:glowing_calorite_pillar'
    );
    const modernIndustrializationTungstensteelCoil = event.memberOfBlock(
        'modern_industrialization:tungstensteel_coil'
    );
    const modernIndustrializationCaloriteMachineCasing = event.memberOfBlock(
        'modern_industrialization:calorite_machine_casing'
    );
    const colossalBlastFurnaceShape = event
        .layeredShape('modern_industrialization:calorite_machine_casing', [
            [' eee ', '     ', '     ', '     ', ' AeA ', '     ', '     '],
