...
MIMachineEvents.registerRecipeTypes(event => {
    ARRAYED_WIREMILL = event.register('arrayed_wiremill')
        .withItemInputs()
        .withItemOutputs();
});
...
MITweaksMachineEvents.registerBatchMultiblocks(event => {
    ...
    const largeWiremillShape = event.layeredShape('clean_stainless_steel_machine_casing', [
        ['AAAAA', 'ACCCA', ' AAA '],
        ['AAAAA', 'BDDDB', 'BBBBB'],
        ['AAAAA', 'ACC#A', ' AAA ']
    ])
        .key('A', modernIndustrializationCleanStainlessSteelMachineCasing, multiblockHatch)
        .key('B', modernIndustrializationStainlessSteelMachineCasingPipe, noHatch)
