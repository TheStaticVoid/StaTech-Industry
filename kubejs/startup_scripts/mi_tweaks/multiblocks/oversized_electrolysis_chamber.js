...
MIMachineEvents.registerRecipeTypes(event => {
    OVERSIZED_ELECTROLYSIS_CHAMBER = event.register('oversized_electrolysis_chamber')
        .withItemInputs()
        .withItemOutputs()
        .withFluidInputs()
        .withFluidOutputs();
...
MITweaksMachineEvents.registerBatchMultiblocks((event) => {
    ...
    const electrolyzerHatch = event.hatchOf('item_input', 'item_output', 'energy_input', 'fluid_input', 'fluid_output');
    ...
    const electrolysischamberShape = event.layeredShape('modern_industrialization:tungstensteel_machine_casing', [
        [' ccc ', '  c  ', '  c  ', '  c  ', ' ccc '],
