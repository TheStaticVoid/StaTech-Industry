...
MIMachineEvents.registerRecipeTypes(event => {
    HIGH_CAPACITY_UNPACKER = event.register('high_capacity_unpacker')
        .withItemInputs()
        .withItemOutputs();
});
...
MITweaksMachineEvents.registerBatchMultiblocks(event => {
  ...
  const highCapacityUnpackerShape = event.layeredShape('modern_industrialization:iron_sheet_metal', [
    ['AAA','BCB','AAA'],
    ['ABA','CDC','AAA'],
    ['A#A','BCB','AAA']
  ])
