...
MIMachineEvents.registerRecipeTypes(event => {
    MULTI_CUTTER = event.register('multi_cutter')
        .withItemInputs()
        .withItemOutputs()
        .withFluidInputs();
});
...
MITweaksMachineEvents.registerBatchMultiblocks(event => {
    ...
    const idenDecorCautionBlockSlab = event.memberOfBlock('iden_decor:caution_block_slab');
    const minecraftStonecutter = event.memberOfBlock('minecraft:stonecutter');
    const multiCutterShape = event.layeredShape('steel', [
        ['ABBBA', 'ABBBA', '     '],
        ['ABBBA', 'ACDCA', 'ACCCA'],
        ['AB#BA', '     ', '     ']
    ])
