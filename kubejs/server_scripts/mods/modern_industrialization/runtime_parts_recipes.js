ServerEvents.recipes((e) => {
    ...
    const MATERIAL_TIERS = [
        {
            tier: 'SOFT',
            materials: ['zinc', 'polytetrafluoroethylene'],
        },
        {
            tier: 'HARD',
            materials: ['desh', 'ostrum'],
        },
        {
            tier: 'VERY_HARD',
            materials: ['calorite', 'enderium'],
        },
        {
            tier: 'EXTREMELY_HARD',
    ...
    TIERS.forEach(function (tier) {
        ...
        MACHINE_FUNCTIONS[name] = {
            ...
            bending: function (output, input, result) {
                return bending(output, 2, duration / 2, input, result);
            },
    ...
    const COMMON_PARTS = [
        ...
        },
        {
            ...
            fluidInputs: [{ amount: 100, fluid: mi('soldering_alloy') }],
            outputAmount: 1,
        },
        {
            suffix: '_rotor',
        ...
        },
        {
            ...
            fluidInputs: [{ amount: 75, fluid: mi('soldering_alloy') }],
            outputAmount: 1,
        },
        {
            suffix: '_cable',
        ...
        tierObj.materials.forEach(function (material) {
            COMMON_PARTS.forEach(function (part) {
                ...
                if (Item.exists(outputId)) {
                    ...
                    let itemInputs = (part.itemInputsPrefix || []).map(
                        function (input) {
                            return {
                                amount: input.amount,
                                tag: input.tag + material,
                            };
                        }
                    );
                    let itemOutputs = [
                        { amount: part.outputAmount, item: outputId },
                    ];
                    ...
                    if (
                        part.machine === 'compressor' &&
                        (part.suffix === '_curved_plate' ||
                            part.suffix === '_ring')
                    ) {
